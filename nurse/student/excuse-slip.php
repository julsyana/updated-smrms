               ...
               <?php 
                if($row['isEmail'] == 1){
                  ?>
                    ...
                    <button id="emailer" disabled> Already Sent </button>
                  <?php

                } else {
                  ?>
                    <button id="emailer"> Send </button>
                  <?php
                }
...
emailer.addEventListener("click", function () {
  ...
  certificate.getBase64((data) => {
    ...
    emailer.innerHTML = "Sending...";
    emailer.disabled = true;
...
function generateCert(name) {
    ...
    stack: [
      {
        ...
        width: 350, // Width of the image in PDF units (A4 page width)
        height: 50, // Height of the image in PDF units
        alignment: "center",
        margin: [0, 20],
      },
    ],
  ...
  var docDefinition = {
    content: [
      ...
      h1,
      {
        text: [{ text: "Date : " }, boldData("<?=$date_today?> \n\n"), ,],
      },
      {
        text: [
          { text: "This is to certify that ", lineHeight: 1.5 },
          boldData("<?=$firstname?> <?=$middleInitial?>. <?=$lastname?>"), //patient Name
          { text: " with student number " },
          boldData("<?=$row['student_id']?>"), //patient Student Number
          { text: " was seen and examined in the clinic on ", lineHeight: 1.5 },
          boldData("<?=$row['campus']?> Campus "), //campus
          { text: "at " },
          boldData("<?=$dateConsult?>"),
          { text: " with complaint of ", lineHeight: 1.5 },
          boldData(
            "<?=$row['symptoms']?> \n\n"
          ), //sickness
          { text: "He/She was advised to; ", lineHeight: 1.5 },
        ],
      },
      {
        columns: [
